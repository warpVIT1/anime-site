...
const SORT_FILTERS = [
  { id: 'score', label: 'Top Rated', sort: ['SCORE_DESC'] },
  { id: 'popularity', label: 'Most Popular', sort: ['POPULARITY_DESC'] },
  { id: 'recent', label: 'Newest', sort: ['START_DATE_DESC'] },
  { id: 'airing', label: 'Currently Airing', sort: ['POPULARITY_DESC'], status: 'RELEASING' },
];
...
export default function TopPage() {
  ...
  useEffect(() => {
    async function loadAnime() {
      ...
      setPage(1);
      try {
        const filter = SORT_FILTERS.find(f => f.id === selectedFilter);
        if (!filter) return;
          ...
          <div className="mb-6 flex flex-col sm:flex-row items-center gap-4">
            ...
            <div className="flex flex-col items-center sm:items-start gap-2">
              ...
              <p className="text-gray-400 text-sm sm:text-base max-w-md">
                Browse the best anime sorted by various criteria
              </p>
            </div>
          </div>
        </div>
